...
Dans git Bash taper la commande ``gpg --default-new-key-algo rsa4096 --gen-key`` pour générer une nouvelle clé, puis ajouter vos informations.
Afficher la liste de vos clé pour récupérer l'ID de la clé ``gpg --list-secret-keys --keyid- format LONG`` qui va donner un résultat comme ceci :
